...
if (!empty($pagina)) {
    ...
    while ($row = $result_usuarios->fetch(PDO::FETCH_ASSOC)) {
        ...
        $viewToken = '<button class="btn btn-info btn-sm vtoken" data-id="' . $token . '" aria-hidden="true" ><i class="fa fa-eye fa-1"></i></button>';

        // data-toggle="modal" data-target="#edittModal"
        $edit = ($_SESSION['usuario']['tipo'] != 2) ? '' : '<button class="btn btn-warning btn-sm etoken" data-id="' . $token . '" ><span class="glyphicon glyphicon-trash"></span> Editar</button>';

        // data-toggle="modal" data-target="#deleteModal"
        $delette = ($_SESSION['usuario']['tipo'] != 2) ? '' : '<button class="btn btn-danger btn-sm deletec" data-id="' . $id_token . '" ><span class="glyphicon glyphicon-trash"></span> Excluir</button>';

        $dados .=  "<tr>
            <td class='text-center' max-width='70px'>" . $numero . "</td>
            <td class='text-left' max-width='220px'>" . $paciente . "</td>
            <td class='text-center' max-width='120px'>" . $token . "</td>
            <td class='text-left' max-width='220px'>" . $profissional . "</td>
            <td class='text-center' max-width='140px'>
            " . $tipopag . "
            </td>
            <td class='text-center'width='110px'>" . $modalidadepag . "</td>
